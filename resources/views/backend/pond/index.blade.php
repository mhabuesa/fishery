            ...
            <div class="col-lg-9 m-auto mt-2">
                <div class="block block-rounded">
                    <div class="block-header block-header-default d-block">
                        <div class="row">
                            <div class="col-lg-6">
                                <h3 class="block-title">Pond List</h3>
                            </div>
                            <div class="col-lg-6 text-center text-lg-end">
                                <div class="block-options space-x-1 md_mt-2 p-0">
                                    <a href="{{route('pond.create')}}" class="btn btn-primary btn-sm"> <i class="fa fa-plus"></i> Add Pond</a>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($ponds as $key => $pond)
                                        <tr>
                                            ...
                                            <td class="fw-semibold fs-sm">{{ $pond->name }}</td>
                                            <td class="fs-sm">{{ $pond->location ?? 'N/A' }}</td>
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-2">
                                                    <a href="{{ route('pond.show', $pond->id) }}"
                                                        class="border-0 btn btn-sm" title="View">
                                                        <i class="fa fa-eye text-primary fa-xl"></i>
                                                    </a>
                                                    <a href="{{ route('pond.edit', $pond->id) }}"
                                                        class="border-0 btn btn-sm" title="Edit">
                                                        <i class="fa fa-pencil text-secondary fa-xl"></i>
                                                    </a>
                                                        <button type="button" class="border-0 btn btn-sm"
                                                            onclick="deletePond(this)" data-id="{{ $pond->id }}" title="Delete"><i
